      ...
      <form action="{{route(config('app.admindomain').'.galleries.update',$gallery->id)}}" method="POST" enctype="multipart/form-data">
        ...
        <div class="grid md:grid-cols-2 grid-cols-1 gap-4">
          <div class="border border-black rounded-xl px-4 py-2 flex flex-wrap md:mt-0 mt-4">
            ...
            <input type="datetime-local" class="en-font outline-none" value="{{old('date',Carbon\Carbon::parse($gallery->date)->format('Y-m-d\TH:i'))}}" placeholder="ތާރިޙް" name="date" />
          </div>
          <div class="border border-black rounded-xl px-4 py-2">
            <p class="w-full en-font rtl text-sm">Heading</p>
            <input type="text" class="dv-bold rtl w-full outline-none" value="{{old('title',$gallery->title)}}" placeholder="ތައާރަފް" name="title" />
            @if($errors->has('title'))
        ...
        <div class="grid md:grid-cols-1 grid-cols-1 gap-4 mt-4">
          <div class="border border-black rounded-xl px-4 py-2 flex flex-wrap justify-end">
            ...
            <textarea type="text" class="dv-bold w-full rtl outline-none h-60" placeholder="ތަފްސީލް" name="description">{{old('description',$gallery->description)}}</textarea>
